...
def build_theme_css(theme: str) -> str:
    ...
    <style>
      ...
      .ecorouter-hero h1 {{ margin: 0; font-size: 2rem; font-weight: 700; letter-spacing: -0.02em; color: {t["hero_text"]}; }}
      .ecorouter-hero p {{ margin: 0.5rem 0 0; color: {t["hero_sub"]}; font-size: 1.02rem; line-height: 1.55; }}
      .ecorouter-pill {{
        ...
        margin-right: 0.45rem; margin-top: 0.65rem; color: {t["hero_text"]};
      }}
      ...
      .tab-intro {{
        ...
        font-size: 0.93rem; color: {t["intro_text"]}; margin-bottom: 1.2rem; line-height: 1.55;
      }}
      .panel-card {{
        background: {t["surface"]}; border: 1px solid {t["border"]}; border-radius: 14px;
        padding: 1rem 1.15rem; margin-bottom: 1rem;
        box-shadow: {t["card_shadow"]}; color: {t["text"]};
      }}

      .carbon-scroll-hint {{
        font-size: 0.8rem; color: {t["text_muted"]}; margin-bottom: 0.5rem; font-weight: 600;
      }}
      .carbon-chart-scroll {{
      ...
      .carbon-bar-value {{
        font-size: 0.82rem; font-weight: 700; color: {t["text"]}; margin-bottom: 6px;
        white-space: nowrap;
      }}
      .carbon-bar-wrap {{
      ...
      .carbon-bar-region {{
        font-size: 0.72rem; font-weight: 700; color: {t["text"]}; margin-top: 8px;
        text-align: center; line-height: 1.2; word-break: break-all;
      }}
      .carbon-bar-label {{ font-size: 0.65rem; color: {t["text_muted"]}; text-align: center; margin-top: 2px; }}
      .carbon-bar-tariff {{
        font-size: 0.68rem; font-weight: 600; color: {t["accent_blue"]}; margin-top: 2px;
      }}
      .bar-badge-green {{
        display: inline-block; background: {t["badge_green_bg"]}; color: {t["badge_green_text"]};
        font-size: 0.6rem; font-weight: 700; padding: 1px 5px; border-radius: 4px; margin-top: 3px;
      }}
      .carbon-legend {{
        margin-top: 0.75rem; font-size: 0.78rem; font-weight: 600; color: {t["text_muted"]};
      }}
      .gradient-legend-bar {{
      ...
      .gradient-legend-labels {{
        display: flex; justify-content: space-between; font-size: 0.72rem; color: {t["text_muted"]};
      }}
      .compare-baseline {{
        color: {t["baseline_text"]}; background: {t["baseline_bg"]};
        padding: 2px 6px; border-radius: 4px; font-weight: 600;
      }}
      .compare-eco {{
        color: {t["eco_text"]}; background: {t["eco_bg"]};
        padding: 2px 6px; border-radius: 4px; font-weight: 600;
      }}
      .compact-grid-row {{ display: flex; align-items: center; gap: 8px; margin-bottom: 6px; font-size: 0.8rem; color: {t["text"]}; }}
      .compact-grid-track {{
      ...
      div.stButton > button[kind="primary"] {{
        ...
        border: none; color: white; font-weight: 700; border-radius: 12px; width: 100%;
...
def assignment_row_colors(theme: str) -> dict[str, tuple[str, str] | str]:
    palette = _ASSIGNMENT_PALETTES["dark" if theme == "dark" else "light"]
    return palette
...
_TOKENS: dict[str, dict[str, str]] = {
    "light": {
        ...
        "intro_bg": "linear-gradient(90deg, #f0fdf4 0%, #f8fafc 100%)",
        "intro_text": "#334155",
        "accent_green": "#15803d",
        "accent_blue": "#0369a1",
        "card_shadow": "0 2px 8px rgba(15, 23, 42, 0.04)",
    ...
    "dark": {
        ...
        "intro_bg": "linear-gradient(90deg, #064e3b 0%, #1e293b 100%)",
        "intro_text": "#e2e8f0",
        "accent_green": "#4ade80",
        "accent_blue": "#38bdf8",
